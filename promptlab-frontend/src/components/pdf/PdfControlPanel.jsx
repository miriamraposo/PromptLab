...
// --- Función de redimensionamiento (esencial para el rendimiento) ---
...
const DocumentControlPanel = (props) => {
  ...
  return (
    <Paper sx={{ display: 'flex', flexDirection: 'column', height: '100%', p: 2, borderRadius: 3, overflowY: 'auto' }}>
      ...
      {/* ===== PESTAÑA 0: GESTOR DE IMÁGENES (AHORA CON CONTENIDO) ===== */}
      <TabPanel value={tabValue} index={0}>
        
        {/* --- ACORDEÓN 1: SUBIR IMAGEN (DROPZONE) --- */}
        <Accordion defaultExpanded>
          <AccordionSummary expandIcon={<ExpandMore />}>
             <CloudUpload sx={{ mr: 1 }} /> <Typography>Subir desde equipo</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Box {...getRootProps()} sx={{ p: 3, border: '2px dashed grey', borderRadius: 2, textAlign: 'center', cursor: 'pointer' }}>
              <input {...getInputProps()} />
              <Typography>Arrastra una imagen o haz clic aquí.</Typography>
            </Box>
          </AccordionDetails>
        </Accordion>

        {/* --- ACORDEÓN 2: MI GALERÍA --- */}
        <Accordion>
          <AccordionSummary expandIcon={<ExpandMore />}>
            <Collections sx={{ mr: 1 }} /> <Typography>Mi Galería</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: 1, maxHeight: '250px', overflowY: 'auto' }}>
              {userImages && userImages.length > 0 ? userImages.map(img => (
                <Box key={img.id} component="img" src={img.url} alt="Imagen de la galería"
                  onClick={() => onImageSelect(img.url)}
                  sx={{ width: '100%', aspectRatio: '1 / 1', objectFit: 'cover', borderRadius: 1, cursor: 'pointer', '&:hover': { opacity: 0.8 } }}
                />
              )) : <Typography variant="body2" color="text.secondary">No tienes imágenes guardadas.</Typography>}
            </Box>
          </AccordionDetails>
        </Accordion>
        
        {/* --- ACORDEÓN 3: PEGAR IMAGEN --- */}
        <Accordion>
          <AccordionSummary expandIcon={<ExpandMore />}>
            <ContentPaste sx={{ mr: 1 }} /> <Typography>Pegar desde Portapapeles</Typography>
          </AccordionSummary>
          <AccordionDetails>
      ...
      <TabPanel value={tabValue} index={1}>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
          <FormControl fullWidth>
            ...
            <Select value={selectedModel || ''} label="Modelo de IA" onChange={e => setSelectedModel(e.target.value)} disabled={isLoading}>
          ...
          <Button variant="contained" onClick={handleGenerateImage} disabled={isLoading} sx={{ flex: 1 }} startIcon={isLoading ? <CircularProgress size={20}/> : <ImageIcon />}>
